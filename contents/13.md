...
- 参数中$match表示匹配state属性为A的数据，也就是说$match是个管道过滤出所有state为A的数据，得到一个集合
- 参数中$group表示根据cust_id属性进行分组，分完组后对属性amount进行求和，也就是说$group也是一个管道，得到我们想要的数据，在这里的$sum是求和的管道表达式
